...
const OrderTable = ({
    ...
    const getStatusColor = (status) => {
        switch (status) {
            ...
            case 'RETURNED': return 'bg-slate-200 text-slate-700';
                ...
                {orders.length === 0 ? (
                    ...
                    orders.map(order => {
                        ...
                        if (order.payments && Array.isArray(order.payments) && order.payments.length > 0) {
                            const successfulPayment = order.payments.find(p => p.status === 'succeeded' || p.status === 'completed' || p.status === 'paid');
                            paymentStatusRaw = successfulPayment ? successfulPayment.status : order.payments[0].status;
                        ...
                        return (
                            ...
                            >
                                <div className="flex justify-between items-start mb-3">
                                    <div>
                                        <span className="font-mono text-xs text-slate-500">#{id.substring(0, 8).toUpperCase()}</span>
                                        <h3 className="font-bold text-slate-800 text-sm">{order.userId?.name || order.user?.name || "Guest"}</h3>
                                    </div>
                                    <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full ${getStatusColor(status)}`}>
                                        {status}
                                    </span>
                                </div>
                                ...
                                <div className="flex justify-between items-center text-sm text-slate-500 mb-3">
                                    ...
                                    <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full ${isPaid ? 'bg-green-50 text-green-600 border border-green-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
                                        {paymentStatus}
                                    </span>
                                </div>

                                <div className="border-t border-slate-100 pt-3 flex justify-between items-center">
                                    <span className="font-bold text-slate-800">₹{order.totalAmount?.toLocaleString() || "0"}</span>
                                    <button
                ...
                <table className="w-full border-collapse min-w-[800px]">
                    <thead>
                        <tr className="bg-slate-50 border-b border-slate-200">
                            <th className="text-left py-3 px-4 text-xs font-semibold text-slate-600 uppercase tracking-wider">Order ID</th>
                            ...
                            <th className="text-left py-3 px-4 text-xs font-semibold text-slate-600 uppercase tracking-wider">Payment</th>
                    ...
                    <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-100 pt-6">
                        <div className="flex items-center gap-4">
                            <p className="text-xs text-slate-500">
                                Showing <span className="font-semibold text-slate-800">{((page - 1) * limit) + 1}</span> to <span className="font-semibold text-slate-800">{Math.min(page * limit, totalItems)}</span> of <span className="font-semibold text-slate-800">{totalItems}</span> orders
                            </p>
                            ...
                            {setLimit && (
                                <select
                                    ...
                                    className="bg-white border border-slate-200 text-slate-600 text-xs rounded-lg focus:ring-blue-500 focus:border-blue-500 p-1.5 outline-none"
                                >
                                    ...
                                    if (
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-8 h-8 flex items-center justify-center rounded-lg border font-medium text-xs transition-all ${page === pageNum
