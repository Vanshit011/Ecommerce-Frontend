            ...
            <table className="w-full border-collapse min-w-[800px]">
                <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                        ...
                        <th className="hidden lg:table-cell text-left py-3 px-4 text-xs font-semibold text-slate-600 uppercase tracking-wider">SKU</th>
                        <th className="hidden md:table-cell text-left py-3 px-4 text-xs font-semibold text-slate-600 uppercase tracking-wider">Category</th>
                    ...
                    {products.length === 0 ? (
                        ...
                        products.map(p => (
                            ...
                            >
                                <td className="py-3 px-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[10px] text-slate-400 uppercase tracking-wider">{p.brand || "No Brand"}</span>
                                        </div>
                                    </div>
                                </td>
                                <td className="hidden lg:table-cell py-3 px-4">
                                    <code className="text-xs text-blue-600 bg-blue-50 px-2 py-0.5 rounded">{p.sku || "N/A"}</code>
                                </td>
                                ...
                                <td className="py-3 px-4">
                                    <div className="flex flex-col">
                                        <span className={`text-sm font-semibold ${p.salePrice ? "text-slate-400 line-through text-xs" : "text-slate-800"}`}>₹{p.price}</span>
                                        {p.salePrice && <span className="text-sm font-bold text-green-600">₹{p.salePrice}</span>}
                                    </div>
                                </td>
                                <td className="py-3 px-4">
                                    <span className={`text-sm font-medium ${p.stockQty < 10 ? "text-red-600" : "text-slate-700"}`}>
                                        {p.stockQty || 0}
                                    </span>
                                </td>
                                <td className="py-3 px-4">
                                    <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full ${p.availability === 'INSTOCK' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'
                                        }`}>
