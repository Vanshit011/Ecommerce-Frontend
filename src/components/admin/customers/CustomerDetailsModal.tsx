...
const CustomerDetailsModal = ({ customer, onClose }) => {
  ...
  const filteredOrders = React.useMemo(() => {
    ...
    if (statusFilter !== "ALL") {
      result = result.filter(
        (o) => (o.orderStatus || o.status || "").toUpperCase() === statusFilter,
      );
    }
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide ${styles[status] || "bg-slate-100 text-slate-600"}`}
      >
        {status}
      </span>
    );
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col animate-fadeIn"
