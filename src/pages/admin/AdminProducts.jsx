...
const AdminProducts = () => {
    ...
    useEffect(() => { fetchProducts(); }, [page, limit, debouncedSearch]);
    ...
    useEffect(() => {
        ...
        if (s !== search) { setSearch(s); setDebouncedSearch(s); }
    }, [searchParams]);

    useEffect(() => {
        const t = setTimeout(() => {
            if (debouncedSearch !== search) updateURL({ search, page: 1 });
        }, 500);
    ...
    const getCategoryPath = (categoryId) => {
        ...
        const flatten = (cats) => {
            let res = [];
            cats.forEach(c => { res.push(c); if (c.children) res.push(...flatten(c.children)); });
            return res;
        };
    ...
    const flattenedForDropdown = (() => {
        ...
        const roots = categories.filter(c => !c.parentId && (!c.parent || (!c.parent.id && !c.parent._id)));
        traverse(roots);
        return result;
    })();
    ...
    const handleSubmit = async (e) => {
        ...
        const append = (k, v) => { if (v !== undefined && v !== null && v !== '') data.append(k, v); };

        ['name', 'description', 'price', 'salePrice', 'sku', 'brand', 'stockQty', 'availability', 'weight', 'metaTitle', 'metaDescription'].forEach(k => append(k, formData[k]));
        if (formData.length || formData.width || formData.height) data.append("dimensions", JSON.stringify({ length: Number(formData.length) || 0, width: Number(formData.width) || 0, height: Number(formData.height) || 0 }));
        ['sizes', 'colors', 'tags'].forEach(k => { if (formData[k]) formData[k].split(",").forEach(v => data.append(`${k}[]`, v.trim())); }); // Fixed brackets
        if (formData.category) data.append("categoryId", formData.category);
        if (formData.image) data.append("images", formData.image);
        if (formData.images?.length > 0) Array.from(formData.images).forEach(f => data.append("images", f));
        ...
        try {
            ...
            if (isEditingId) {
                ...
                setProducts(prev => prev.map(p => (String(p.id || p._id) === String(isEditingId)) ? updated : p));
            } else {
        ...
        <div className="p-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-slate-600 mt-1 text-sm lg:text-base">Manage your product catalog and stock levels.</p>
                </div>
                <button
