      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-8 md:p-12 text-center border border-slate-100 relative overflow-hidden">
          ...
          <div className="absolute inset-0 pointer-events-none opacity-10">
            <div className="absolute top-10 left-10 w-4 h-4 bg-blue-500 rounded-full animate-bounce" style={{ animationDelay: '0s' }} />
            <div className="absolute top-20 right-20 w-3 h-3 bg-red-500 transform rotate-45 animate-bounce" style={{ animationDelay: '1s' }} />
            <div className="absolute bottom-10 left-1/4 w-2 h-2 bg-green-500 rounded-full animate-bounce" style={{ animationDelay: '2s' }} />
          </div>
          ...
          <p className="text-slate-500 font-medium text-lg mb-8 max-w-lg mx-auto leading-relaxed">
            Thank you for your purchase. We've received your order <span className="text-slate-900 font-bold">#{order.id?.slice(0, 8)}</span> and sent a confirmation email.
          </p>
          ...
          <div className="max-w-md mx-auto bg-slate-50 rounded-3xl p-6 md:p-8 text-left border border-slate-100">
            <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-6 border-b border-slate-200 pb-2">Receipt</h3>

            <dl className="space-y-4 text-sm">
              <div className="flex justify-between">
                <dt className="text-slate-500 font-medium">Payment Method</dt>
                <dd className="font-bold text-slate-900 capitalize">{order.paymentMethod || "Online Card"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-slate-500 font-medium">Date</dt>
                <dd className="font-bold text-slate-900">{new Date(order.created_at || Date.now()).toLocaleDateString()}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-slate-500 font-medium">Email</dt>
                <dd className="font-bold text-slate-900">{order.user?.email || "Sent to your email"}</dd>
              </div>
              ...
              <div className="border-t border-slate-200 my-4 pt-4 flex justify-between items-end">
                ...
                <dd className="font-black text-blue-600 text-2xl tracking-tighter">₹{Number(order.totalAmount || order.totalPrice).toLocaleString()}</dd>
              </div>
            </dl>
            ...
            {order.address && (
              <div className="mt-6 pt-6 border-t border-slate-200">
                ...
                <p className="text-slate-500">{order.address.addressline1}, {order.address.city}</p>
              </div>
            )}
          </div>
        </div>
