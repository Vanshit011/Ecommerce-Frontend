...
const OrderSuccess: React.FC = () => {
  ...
  const [displayDate] = useState<number>(() => Date.now());
  ...
  useEffect(() => {
    if (id) {
      getOrderById(id).then((res) => {
        ...
        const data = (res.data as any).data || (res.data as any) || res.data;
        setOrder(data);
      });
    }
  }, [id]);
  ...
  const paymentMethod = (order as any).paymentMethod || "Online Card";
  const createdAt = order.created_at || order.createdAt;
  const totalAmount = order.total_amount || order.totalAmount || (order as any).totalPrice;
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-8 md:p-12 text-center border border-slate-100 relative overflow-hidden">
          ...
          <p className="text-slate-500 font-medium text-lg mb-8 max-w-lg mx-auto leading-relaxed">
            ...
            <span className="text-slate-900 font-bold">#{orderId.slice(0, 8)}</span> and sent a
            confirmation email.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
            <button
              ...
              className="w-full sm:w-auto px-8 py-3.5 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-all active:scale-95 shadow-lg shadow-blue-200"
            >
              View My Order
            </button>
            <button
          ...
          <div className="max-w-md mx-auto bg-slate-50 rounded-3xl p-6 md:p-8 text-left border border-slate-100">
            ...
            <dl className="space-y-4 text-sm">
              <div className="flex justify-between">
                ...
                <dd className="font-bold text-slate-900 capitalize">{paymentMethod}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-slate-500 font-medium">Date</dt>
                <dd className="font-bold text-slate-900">
                  {new Date(createdAt || displayDate).toLocaleDateString()}
                </dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-slate-500 font-medium">Email</dt>
                <dd className="font-bold text-slate-900">
                  {(order.user as any)?.email || "Sent to your email"}
                </dd>
              </div>
