    ...
    const loadData = async () => {
      try {
        ...
        const buildCategoryTree = (cats: any[]): CategoryTree[] => {
          ...
          const attachChildren = (parent: CategoryTree): CategoryTree => {
            const children = cats.filter((cat) => {
              const pid = (parent as any).id || parent._id;
              const parentIdMatch = cat.parentId === pid;
              const parentObjMatch =
                cat.parent &&
                typeof cat.parent === "object" &&
                (cat.parent.id === pid || cat.parent._id === pid);
        ...
        const featuredWithStats = await Promise.all(
          featuredSlice.map(async (p): Promise<ProductWithStats> => {
            try {
        ...
        const bestSellersWithStats = await Promise.all(
          bestSellersSlice.map(async (p): Promise<ProductWithStats> => {
            try {
              const pid = (p as any).id || p._id;
              const statsRes = await getProductStats(pid);
              const stats = statsRes.data?.data || statsRes.data;
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="relative w-full overflow-hidden pb-6 pt-2">
            ...
            <div className="flex gap-4 animate-marquee hover:[animation-play-state:paused]">
              {(categories.length > 0 ? [...categories, ...categories] : []).map((cat, index) => (
                <div
                  key={`${(cat as any).id || cat._id}-${index}`}
                  onClick={() => navigate(`/products?category=${(cat as any).id || cat._id}`)}
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
            {loading
              ...
              : featured.map((p) => (
                  <div
                    key={(p as any).id || p._id}
                    onClick={() => navigate(`/product/${(p as any).id || p._id}`)}
                  ...
                  >
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <StarRating
                          rating={Math.round(
                            p.averageRating || (p as any).average_rating || (p as any).average || 0,
                          )}
                          size="sm"
                        />
                        <span className="text-[10px] text-slate-400 font-bold">
                          ({p.reviewCount || (p as any).total_reviews || (p as any).count || 0})
                        </span>
                      </div>
