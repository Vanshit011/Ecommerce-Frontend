...
const Profile: React.FC = () => {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      ...
      // res.data is expected to be ApiResponse<User>, where res.data.data is the User
      const user =
  ...
  const loadAddresses = useCallback(async () => {
    ...
    try {
      ...
      const addressList = (res.data as any).data || (Array.isArray(res.data) ? res.data : []);
  ...
  const handleAddressSubmit = async () => {
    try {
      ...
      if (editingAddress) {
        await updateAddress(editingAddress._id || (editingAddress as any).id, payload);
  ...
  const handleEditAddress = (addr: Address) => {
    ...
    setAddressForm({
      ...
      addressline1: addr.address_line_1 || addr.addressline1 || addr.addressLine1 || "",
      addressline2: addr.address_line_2 || (addr as any).addressline2 || addr.addressLine2 || "",
      city: addr.city || "",
      state: addr.state || "",
      postalcode: addr.postal_code || addr.postalcode || addr.postalCode || "",
  ...
  const handleSetDefault = async (id: string) => {
    const alreadyDefault = addresses.find(
      (a) => (a._id || (a as any).id) === id && (a.is_default || a.isdefault || a.isDefault),
    );
          ...
          <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6">
            <div className="flex items-center gap-4 mb-8 px-2">
              <div className="w-12 h-12 bg-blue-600 text-white rounded-2xl flex items-center justify-center text-xl font-black shadow-lg shadow-blue-200">
                {(profile?.name?.[0] || profile?.firstName?.[0] || "U").toUpperCase()}
              </div>
                  ...
                  >
                    ...
                    <div className="pr-16">
                      ...
                      <p className="text-sm text-slate-600 leading-relaxed">
                        {addr.address_line_1 || addr.addressline1 || addr.addressLine1}
                        {(addr.address_line_2 ||
                          (addr as any).addressline2 ||
                          addr.addressLine2) && (
                          <>
                            ,{" "}
                            {addr.address_line_2 || (addr as any).addressline2 || addr.addressLine2}
                          </>
                        )}
                        <br />
                        {addr.city}, {addr.state} -{" "}
                        <span className="font-semibold text-slate-800">
                          {addr.postal_code || addr.postalcode || addr.postalCode}
                        </span>
                        <br />
