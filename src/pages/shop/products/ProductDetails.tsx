...
const ProductDetails: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const checkFavoriteStatus = async () => {
      ...
      try {
        ...
        const favs = ((res.data as any).data || res.data || []) as any[];
        const found = favs.some(
          (f) =>
            String(f.product?.id || f.id || f.product?._id || f._id || f.productId) === String(id),
        );
        setIsFavorite(found);
      } catch (err) {
  ...
  const handleAddToCart = async () => {
    if (!validateSelection() || !product || !id) return;
  ...
  return (
    <div className="bg-slate-50 min-h-screen animate-fade-in">
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-8 md:py-12">
                  ...
                  if (product?.images && Array.isArray(product.images)) {
                    product.images.forEach((img) => {
                      ...
                      const isDup = gallery.some(
                        (g) => (typeof g === "string" ? g : (g as any).url) === imgUrl,
                      );
              ...
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-3 mb-6">
                  <span className="px-4 py-1.5 bg-blue-50 text-blue-600 text-[10px] font-black uppercase tracking-[0.2em] rounded-full border border-blue-100/50">
                    {(product?.category as any)?.name || "Premium Collection"}
                  </span>
                  {product && hasVariants(product) ? (
                  ...
                  ) : product && (product?.stock_qty || product?.stockQty || 0) > 0 ? (
                    <div className="flex items-center gap-1.5 px-4 py-1.5 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase tracking-[0.2em] rounded-full border border-emerald-100/50">
                      <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                      In Stock
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-[1.2fr_1fr] gap-4">
                  <button
                    ...
                    disabled={
                      product && hasVariants(product)
                        ? !selectedVariant || !isVariantAvailable(selectedVariant)
                        : (product?.stock_qty || product?.stockQty || 0) <= 0 || buyingNow
                    }
                  ...
                  <button
                    ...
                    disabled={
                      product && hasVariants(product)
                        ? !selectedVariant || !isVariantAvailable(selectedVariant)
                        : (product?.stock_qty || product?.stockQty || 0) <= 0 || addingToCart
                    }
        ...
        <div id="reviews-section">{id && <ReviewSection productId={id} />}</div>

        {recommendations.length > 0 && <RecommendedProducts products={recommendations} />}
      </div>
    </div>
  );
};
